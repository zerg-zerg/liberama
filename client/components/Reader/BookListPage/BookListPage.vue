<template>
    <div ref="main" class="column no-wrap" style="min-height: 500px; max-height: 100%">
        <div class="col-auto column justify-start items-start no-wrap overflow-hidden" style="padding-left:5%">
            <div class="row justify-start items-center no-wrap overflow-hidden" style="width: 773px; padding: 6px 0; gap: 16px;max-width: 100%;min-width: 400px;">
                ...
                <div style="display:flex">
                    <div v-show="needBookUpdateCount > 0" class="row justify-center items-center"
                    ...
                    <div class="row justify-center items-center"
                        :class="{ 'header-button': !showArchive, 'header-button-pressed': showArchive }" @mousedown.stop
                        @click="showArchiveToggle">
            ...
            <div id="vs-container" ref="vsContainer" class="recent-books-scroll col">
                <div ref="header" class="scroll-header row bg-header-3">
                    <q-btn class="tool-button" color="btn2" round @click="showSameBookClick">
                        <q-icon name="la la-caret-right" class="icon" :class="{ 'expanded-icon': showSameBook }"
                            color="green-8" size="24px" />
                        <q-tooltip :delay="1500" anchor="bottom middle" content-style="font-size: 80%">
                    ...
                    <q-input ref="input" v-model="search" class="q-ml-sm q-mt-xs" outlined dense style="width: 185px"
                        bg-color="input" placeholder="Найти" @click.stop>
                        <template #append>
                            <q-icon v-if="search !== ''" name="la la-times" class="cursor-pointer"
                                @click.stop="resetSearch" />
                        </template>
                    </q-input>

                    <q-select ref="sortMethod" v-model="sortMethod" class="q-ml-sm q-mt-xs" :options="sortMethodOptions"
                        style="width: 180px" bg-color="input" dropdown-icon="la la-angle-down la-sm" outlined dense
                        emit-value map-options display-value-sanitize options-sanitize options-html display-value-html
                    ...
                    <div class="table-row row"
                        ...
                        <div class="row-part column justify-center items-stretch" style="width: 54px">
                            <div class="col row justify-start items-center clickable" style="padding: 0 2px 0 2px"
                                ...
                                <div v-show="bothBucEnabled && item.needBookUpdate" class="column justify-center"
                            ...
                            <div v-show="!showSameBook && item.group && item.group.length > 0"
                                class="row justify-center" style="font-size: 70%">
                                {{ (item.group ? item.group.length + 1 : 0) }} верси{{ wordEnding((item.group ?
                                    item.group.length +
                                    1 : 0), 1) }}
                            </div>
                        </div>
                        ...
                        <div class="row-part column items-stretch clickable break-word bottom-border" @click="loadBook(item)">
                            <div class="col "
                                ...
                                <div v-show="bothBucEnabled && item.needBookUpdate"
                                    style="font-size: 75%; color: blue;">
                                    Размер: {{ item.bucSize - item.downloadSize > 0 ? '+' : '' }}{{ item.bucSize -
                                        item.downloadSize
                                    }}
                            ...
                            <div class="row" style="font-size: 10px;justify-content: flex-end;display: flex;align-items: flex-end;">
                                ...
                                <div class="col row" style="flex-wrap: nowrap; ">
                                    ...
                                    <div class="row justify-start items-center row-info-top time-info"
                                        style="padding-right: 16px">
                        ...
                        <div class="row-part column bottom-border" style="width: 40px;">
                            ...
                            <div v-show="bothBucEnabled && item.showCheckBuc" class="buc-checkbox self-start">
                                <q-checkbox v-model="item.checkBuc" size="xs"
                                    style="position: relative; top: -3px; left: -3px;"
                                    @update:model-value="checkBucChange(item)">
                                    <q-tooltip :delay="1500" anchor="bottom middle" content-style="font-size: 80%">
                                        <div v-if="item.checkBuc === undefined">
                                            Проверка обновлений отключена автоматически<br>т.к. книга не обновлялась {{
                                                bucCancelDays }} дней
                                        </div>
                                        <div v-else>
        ...
        <div class="col column justify-end items-center no-wrap overflow-hidden">
            ...
            <span v-else class="bottom-span">Версия сервера {{ version }}, версия клиента {{ clientVersion }},
                необходимо обновить страницу</span>
        </div>
    </div>
</template>
...
class BookListPage {
    ...
    get header() {
        ...
        return `${(this.search || this.showNeedBookUpdateOnly ? `Найден${this.wordEnding(len, 2)}` : ' ')} ${len} файл${this.wordEnding(len)}${this.showArchive ? ' в архиве' : ''}`;
    }
...
export default vueComponent(BookListPage);
//-----------------------------------------------------------------------------
</script>
<style scoped>
.greeting {
